...
void myinit()
{
    glClearColor(1,1.5,1,1); //这句话设置了背景的颜色。
}
...
void setupScenario(RVO::RVOSimulator *sim)
{
    ...
    for (size_t i = 0; i < 5; ++i) //5*5*4(每个地区25个？
    {
		for (size_t j = 0; j < 5; ++j)
		{
			sim->addAgent(RVO::Vector2(55.0f + i * 10.0f,  55.0f + j * 10.0f));
			goals.push_back(0);//0号终点？

			sim->addAgent(RVO::Vector2(-55.0f - i * 10.0f,  55.0f + j * 10.0f));
			goals.push_back(1);//1号终点
